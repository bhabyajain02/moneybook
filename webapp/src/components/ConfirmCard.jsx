            ...
            return (
              <tr key={i} className="nb-row-data" style={{ borderLeftColor: color }}>
                <td className="nb-cell">
                  <span className="nb-type-pill" style={{ background: color + '1a', color }}>
                    {typeLabel}
                  </span>
                  <span className="nb-txn-desc">{txn.description || '—'}</span>
                  {txn.tag && (
                    <span className="nb-txn-tag">🏷️ {txn.tag.replace(/_/g, ' ')}</span>
                  )}
                  {txn.person_name && (
                    <span className="nb-txn-person"> · {txn.person_name}</span>
                  )}
                </td>
                ...
                <td className="nb-actions-col nb-actions-sticky">
                  ...
                  <button className="nb-btn" onClick={() => setEditingIdx(i)} title="Edit">✏️</button>
                  <button className="nb-btn" onClick={() => onDelete(i)} title="Delete">🗑️</button>
                </td>
              </tr>
            )
          })}
    ...
    return (
      <div className="txn-card" style={{ borderLeftColor: color }}>
        <div className="txn-card-row">
          <span className="txn-num" style={{ background: color }}>#{index + 1}</span>
          <span className="txn-type-badge" style={{ background: color + '18', color }}>{typeLabel}</span>
          <span className="txn-amount">₹{parseFloat(txn.amount).toLocaleString('en-IN')}</span>
          <ConfPill value={txn.confidence} />
          <button className="txn-edit-btn" onClick={() => setEditing(true)} title="Edit">✏️</button>
          <button className="txn-delete-btn" onClick={onDelete} title="Delete">🗑️</button>
        </div>
        ...
        <div className="txn-footer-row">
          ...
          <span className="txn-mode">{modeIcon} {txn.payment_mode || 'cash'}</span>
        </div>
      </div>
    )
  }
...
function AddEntryForm({ onAdd, onCancel }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      onAdd({ type:'expense', amount: parseFloat(amount), description: desc,
              tag:'other', person_name: person || null,
              needs_tracking: !!person, payment_mode:'cash',
  ...
  return (
    <div className="add-entry-form">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <input className="add-entry-input" type="number" inputMode="decimal"
          placeholder="Amount ₹" value={amount} onChange={e => setAmount(e.target.value)} required />
        <input className="add-entry-input" type="text" placeholder="Person (optional)"
        ...
        <div className="add-entry-actions">
          ...
          <button type="button" className="txn-cancel-btn" onClick={onCancel}>Cancel</button>
        </div>
      </form>
    </div>
  )
    ...
    <div className="confirm-card-wrapper">
      ...
      {(totalIn > 0 || totalOut > 0) && (
        <div className="confirm-summary-bar">
          <span>📥 In: <b style={{color:'#25D366'}}>{fmtRs(totalIn)}</b></span>
          <span>📤 Out: <b style={{color:'#E53935'}}>{fmtRs(totalOut)}</b></span>
        </div>
      )}
