...
export async function pollMessages(phone, afterId = 0) {
  const r = await fetch(
    `${BASE}/messages?phone=${encodeURIComponent(phone)}&after_id=${afterId}`,
  );
...
export async function fetchExpenseCategories(phone) {
  const r = await fetch(
    `${BASE}/expense-categories?phone=${encodeURIComponent(phone)}`,
  );
...
export async function confirmTransactions(
  ...
  if (!r.ok)
    throw new Error(
      data.detail ? JSON.stringify(data.detail) : `Server error (${r.status})`,
    );
  return data;
}
...
export async function deleteTransaction(phone, txnId) {
  const r = await fetch(
    `${BASE}/transaction?phone=${encodeURIComponent(phone)}&txn_id=${txnId}`,
    {
      method: "DELETE",
    },
  );
...
export async function fetchPersonDuesHistory(phone, personName) {
  const r = await fetch(
    `${BASE}/dues/person?phone=${encodeURIComponent(phone)}&name=${encodeURIComponent(personName)}`,
  );
...
export async function dismissMessage(phone, botMessageId) {
  const r = await fetch(
    `${BASE}/dismiss?phone=${encodeURIComponent(phone)}&bot_message_id=${botMessageId}`,
    {
      method: "POST",
    },
  );
...
export async function speakLedger(
  inEntries,
  outEntries,
  dateStr,
  language = "hinglish",
...
export async function adminGetQueue(status = "pending") {
  const r = await fetch(`${BASE}/admin/queue?status=${status}`, {
...
export async function adminPickQueue(queueId, operatorId = "default") {
  const r = await fetch(
    `${BASE}/admin/queue/${queueId}/pick?operator_id=${operatorId}`,
    {
      method: "POST",
      headers: getAdminHeaders(),
    },
...
export async function adminPollQueue(since) {
  const url = since
    ? `${BASE}/admin/queue/poll?since=${since}`
    : `${BASE}/admin/queue/poll`;
...
export async function adminGetDescriptions(storeId, type) {
  const r = await fetch(
    `${BASE}/admin/descriptions?store_id=${storeId}&type=${encodeURIComponent(type)}`,
    {
      headers: getAdminHeaders(),
    },
  );
